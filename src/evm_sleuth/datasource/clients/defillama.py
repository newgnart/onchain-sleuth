...
class DeFiLlamaClient(BaseAPIClient):
    ...
    def __init__(self, calls_per_second: float = 10.0):  # DeFiLlama usually has higher limits
        config = APIConfig(
            base_url=settings.api_urls.DEFILLAMA_API,
    ...
    def get_token_price(self, network: str, contract_address: str, **params) -> Dict[str, Any]:
...
class DeFiLlamaDLTResource(BaseDLTResource):
    ...
    def create_dlt_source(self, base_url: str, endpoint: str, data_selector: str, params: Optional[Dict] = None):
    ...
    def stables_metadata(self) -> Iterator[TDataItems]:
        ...
        def _process_chain_balances(response: Dict, metadata: Dict) -> Iterator[Dict]:
            ...
            for chain_name, chain_data in response.get("chainBalances", {}).items():
                for entry in chain_data.get("tokens", []):
                    ...
                    self.data_transformer.standardize_item(item, {"timestamp_fields": ["timestamp"]})
                    yield item
        ...
        for response in source:
            ...
            for item in time_series_data:
                ...
                if data_selector == "totalDataChart":
                    ...
                    self.data_transformer.standardize_item(revenue_item, {"timestamp_fields": ["timestamp"]})
                    yield revenue_item
    ...
    @dlt.resource
    def all_yield_pools(self) -> Iterator[TDataItems]:
        """Get the latest data for all yield pools."""
        source = self.create_dlt_source(
            settings.api_urls.DEFILLAMA_YIELDS, 
            "pools", 
            "data"
        )
    ...
    def yield_pool(self, pool_id: str, pool_name: str) -> Iterator[TDataItems]:
